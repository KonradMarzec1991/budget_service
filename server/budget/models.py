...
class Budget(DatedModel):
    ...
    def calculate_balance(self):
        qs = Budget.objects.filter(id=self.id).annotate(
    ...
    def aggregate_by_expense_category(self):
        ...
        for exp in agg_expenses:
            category_stats.append(
                {
                    "category": exp["expenses__category"],
                    "total": str(exp["total"]),
                }
            )
...
class Expense(DatedModel):
    ...
    category = models.CharField(
        max_length=30, choices=CategoryName.choices, null=False
    )
    amount = models.DecimalField(
        default=Decimal("0.00"), max_digits=8, decimal_places=2
    )
...
@receiver(
    post_delete, sender=Expense, dispatch_uid="expense_item_post_delete_signal"
)
